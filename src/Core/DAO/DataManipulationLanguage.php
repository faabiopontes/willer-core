...
namespaceCoreDAO
	...
	abstract class DataManipulationLanguage
		...
		function __construct(CoreDAOTransaction $transaction)
			{
			...
			if (!$transaction instanceof CoreDAOTransaction)
				{
			...
			function getTableName():
				...
				function getTableColumn():
					...
					function getTableSchema():
						 ...
						 */
						protected
						function getTransaction(): ? CoreDAOTransaction
								{
							 ...
							 */
							protected
							function setTransaction(CoreDAOTransaction $transaction) : self
								{
							...
							function getLimit(): ? string
								 ...
								 */
								private
								function setLimit( ? string $limit) : self
									{
									$this->limit = $limit;
									return $this;
									}
								 ...
								 */
								private
								function setLimitValue(int $page, int $limit) : self
									{
									$this->limit_value = ['page' => $page, 'limit' => $limit, ];
									return $this;
									}
								...
								function getOrderBy(): ? array
									 ...
									 */
									private
									function setOrderBy( ? array $order_by) : self
										{
									 ...
									 */
									private
									function setPrimaryKey( ? string $column) : self
										{
									 ...
									 */
									private
									function setLastInsertId( ? int $id) : self
										{
									 ...
									 */
									private
									function setWhereUnique( ? array $where_unique) : self
										{
									 ...
									 */
									private
									function getWhereUniqueValue() : ? array
										{
									 ...
									 */
									private
									function setWhereUniqueValue( ? array $where_unique_value) : self
										{
									 ...
									 */
									private
									function setWhere( ? array $where) : self
										{
										$this->where = $where;
										return $this;
										}
									 ...
									 */
									private
									function setWhereValue( ? array $where_value) : self
										{
									 ...
									 */
									private
									function setBetween( ? array $between) : array
										{
									 ...
									 */
									private
									function getBetweenValue() : ? array
										{
									 ...
									 */
									private
									function setBetweenValue( ? array $between_value) : self
										{
									 ...
									 */
									private
									function setLike( ? array $like) : self
										{
										$this->like = $like;
										return $this;
										}
									 ...
									 */
									private
									function setLikeValue( ? array $like_value) : self
										{
									 ...
									 */
									private
									function setQuery(string $sql, ? array $value) : self
										{
										$this->query[] = ['sql' => $sql, 'value' => $value];
										return $this;
										}
									 ...
									 */
									protected
									function definePrimaryKey() : self
										{
									...
									function orderBy(array $order_by):
										...
										function limit(int $page, int $limit):
											self
												{
												if (empty($limit))
													{
											 ...
											 */
											private
											function related(stdClass $table_related, array $query_list = [], ? string $join = null, ? string $table_related_name_alias = null) : array
												{
											...
											function where(array $where) : self
												{
												$where_value_list = [];
												if (empty($where))
													{
											...
											function like(array $like, ? string $direction = null) : self
												{
												$like_value_list = [];
												if (empty($like))
													{
													...
													if (!empty($direction))
														...
														if ($direction == self::LIKE_DIRECTION_RIGHT)
															{
															$like_value_str = '%s%%';
															}
														}
											...
											function between(array $between):
												self
													{
													$between_value_list = [];
													if (empty($between))
												...
												function get(array $where):
													self
														...
														foreach($table_column as $column => $value)
															...
															$object_value = $this->$method(null,
															function () use($pdo_query_fetch, $table_column_str)
																{
													...
													function save( ? array $column = null) : self
														{
														$transaction = $this->getTransaction();
														if (empty($transaction))
															{
														...
														if (empty($flag_getdiscard))
															...
															$this->get([vsprintf('%s.id', [$table_name_with_escape, ]) => $last_insert_id]);
															}
													...
													function update(array $set):
														...
														function delete( ? array $where = null) : self
															{
															$transaction = $this->getTransaction();
															if (empty($transaction))
																{
														...
														function execute( ? array $setting = null) : stdClass
															{
															$transaction = $this->getTransaction();
															if (empty($transaction))
																{
															...
															$query_total = vsprintf('SELECT COUNT(1) total FROM %s %s %s %s %s %s', [$table_name_with_escape, $related_join, $where_implicit, $where, $between, $like]);
															$this->setQuery($query_total, $query_value);
															$query = vsprintf('SELECT %s FROM %s %s %s %s %s %s %s %s', [$column_list, $table_name_with_escape, $related_join, $where_implicit, $where, $between, $like, $order_by, $limit]);
															$this->setQuery($query, $query_value);
															try
																{
																$pdo_query_total = $transaction_resource->prepare($query_total);
														 ...
														 */
														private
														function relatedFetch(array $obj_column_list, array $obj_schema_dict, stdClass $fetch, CoreDAOTransaction $transaction, stdClass $obj):
															stdClass
																{
																...
																foreach($obj_column_list as $column => $value)
																	...
																	if ($method == 'foreignKey')
																		...
																		foreach($obj_foreignkey_column_list as $column_recursive => $value_recursive)
																			...
																			$object_value = $this->$method_recursive(null,
																			function () use($fetch, $table_column)
																				{
